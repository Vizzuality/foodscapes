...
def main(files: list[Path], description_file: Path, output_path: Path):
    ...
    # Pivot DataFrame
    filter_columns = list(description_table[description_table['section'].isin(['foodscapes', 'risks', 'locations'])]['widget_column'])
    ...
    pivot = pd.concat([pivot_count, pivot_mean.drop(columns=filter_columns), pivot_sum.drop(columns=filter_columns)],
                      axis=1)
    ...
    pivot.reset_index().rename(columns={'index': 'id'}).to_csv(os.path.join(
        output_path, f'foodscapes_filter_table_{day}_00.csv'), index=False)
    pivot.reset_index().rename(columns={'index': 'id'}).to_parquet(os.path.join(
        output_path, f'foodscapes_filter_table_{day}_00.parquet'), index=False)
